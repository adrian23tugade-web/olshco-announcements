...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($title) || empty($content)) {
        ...
        $stmt = $pdo->prepare("
            INSERT INTO announcements (title, content, category, priority, expiration_date, status, created_by) 
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ");
        
        if ($stmt->execute([$title, $content, $category, $priority, $expiration, $status, $_SESSION['user_id']])) {
    ...
    <div class="admin-wrapper">
        ...
        <div class="admin-main">
            ...
            <div class="admin-topbar">
                <h1 class="page-title">
                    <i class="fas fa-plus-circle me-2" style="color: #800000;"></i>
                    ...
                    <form method="POST" action="">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea class="form-control" id="content" name="content" rows="8" 
                                      placeholder="Enter announcement content..." required><?= htmlspecialchars($_POST['content'] ?? '') ?></textarea>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select" id="category" name="category">
                                    <option value="Announcement" <?= ($_POST['category'] ?? '') == 'Announcement' ? 'selected' : '' ?>>Announcement</option>
                                    <option value="Event" <?= ($_POST['category'] ?? '') == 'Event' ? 'selected' : '' ?>>Event</option>
                                    <option value="Bulletin" <?= ($_POST['category'] ?? '') == 'Bulletin' ? 'selected' : '' ?>>Bulletin</option>
                                    <option value="News" <?= ($_POST['category'] ?? '') == 'News' ? 'selected' : '' ?>>News</option>
                                </select>
                            </div>
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <a href="/dashboard" class="btn btn-outline-primary btn-lg ms-auto">
